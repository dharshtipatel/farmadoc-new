...
type TopDealsProps = {
  deals?: Deal[]; // optional, if you want to generate random products when not passed
};
...
export default function TopDeals({ deals }: TopDealsProps) {
  ...
  const [allProducts, setAllProducts] = useState<Deal[]>(deals || []);
  const [visibleProducts, setVisibleProducts] = useState<Deal[]>(deals?.slice(0, 9) || []);
